int multi_max(int arr[8][8], int depth);
int multi_min(int arr[8][8], int depth);

int one_max(int arr[8][8])
{
    int arr2[8][8];
...
int multi_max(int arr[8][8], int depth)
{
    ...
    for (int i: v) {
        ...
        int sc;
        if (depth == 2) sc = one_min(arr2);
        if (depth > 2) sc = multi_min(arr2, depth-1);
...
int multi_min(int arr[8][8], int depth)
{
    ...
    for (int i: v) {
        ...
        int sc; 
        if (depth == 2) sc = one_max(arr2);
        if (depth > 2) sc = multi_max(arr2, depth-1);
        minscore = min(minscore, sc);
    }
